...
\subsubsection{Uses}
Browser Module (M2)\\
System API Gateway Module (M10)\\
Graphing Module (M12)\\
File Import Module (M13)\\
File Export Module (M14)\\
Repository API Module (M15)
...
\subsubsection{Module}
Handles user-initiated edits to domain, package, and metric data. Provides client-side validation and coordination of save/delete operations through the System API Gateway.

\subsubsection{Uses}
Application UI Module (M3)\\
User Authentication Module (M5)\\
User Role Access Module (M6)\\
System API Gateway Module (M12) [Django REST / JSON] \\
...
\begin{itemize}
  ...
  \item Network transport to backend via M12 (Django REST over HTTPS, JSON).
\end{itemize}

\paragraph{Assumptions}
\begin{itemize}
  \item The user is authenticated (M5) and authorized (M6) for the requested action.
  \item Requests and responses use JSON and follow the schemas exposed by M12.
  \item Persistent storage of edits is performed by M7 behind M12.
\end{itemize}
...
\begin{itemize}
  \item transition: validate \texttt{metricData}; stage in \texttt{editBuffer}; send create request via M12 to M7 on commit.
...
\begin{itemize}
  ...
  \item output: \texttt{successFlag} = true if deletion is confirmed by M7.
...
\begin{itemize}
  \item transition: send up to \texttt{MAX\_EDIT\_BATCH} staged changes via M12; clear successful items from \texttt{editBuffer}; push entries to \texttt{undoStack}.
  \item output: \texttt{successFlag} = true if all changes are acknowledged by M7.
...
\subsubsection{Module}
Manages user registration, login, logout, and session validation. Provides secure handling of credentials and tokens, ensuring that only authorized users can access protected data and functionality in the DomainX web application.

\subsubsection{Uses}
System API Gateway Module (M12) [Django REST Framework, JSON]\\
User Role Access Module (M6)\\
Database Module (M8) [Django ORM]\\
Application UI Module (M3)
...
hashPassword & password & hashedPassword & None \\
\bottomrule
\end{tabularx}
\end{center}
...
\paragraph{Environment Variables}
\begin{itemize}
  \item Django authentication middleware and token management system.
  \item Encrypted database storage via M8 (using Django ORM).
  \item HTTPS communication channel for login and registration requests.
\end{itemize}

\paragraph{Assumptions}
\begin{itemize}
  \item Authentication relies on Django’s built-in \texttt{AbstractUser} model and REST Framework’s token authentication.
  \item All credential transmissions occur via HTTPS to prevent interception.
  \item The UI (M3) handles local validation before requests are sent.
  \item Tokens follow the JWT format and expire after \texttt{TOKEN\_EXPIRY\_HOURS}.
\end{itemize}
...
\begin{itemize}
  \item transition: create a new user record in M8 with hashed password and assigned role.
...
\begin{itemize}
  \item transition: validate credentials; create new session token and add to \texttt{activeSessions}.
...
\begin{itemize}
  ...
  \item output: Boolean indicating if token exists and is unexpired.
  \item exception: \texttt{TokenError} if malformed or tampered.
\end{itemize}

\noindent \textbf{refreshToken}(oldToken):
\begin{itemize}
  \item transition: invalidate old token; issue new one with reset expiry.
  \item output: \texttt{newToken}.
  \item exception: \texttt{TokenExpiredError} if token already expired.
\end{itemize}

\noindent \textbf{hashPassword}(password):
\begin{itemize}
  \item transition: none (pure function using Django’s \texttt{make\_password()}).
  \item output: securely hashed string.
  \item exception: none.
\end{itemize}
...
\begin{itemize}
  ...
  \item \texttt{syncWithBackend()} – pushes updates to Django ORM via M12.
\end{itemize}
...
\begin{itemize}
  ...
  \item Network communication with Django REST API via M12 (HTTPS, JSON payloads).
  \item Persistent user data stored in database via M8 (Django ORM).
\end{itemize}
...
\begin{itemize}
  \item transition: updates local and backend profile data via M12.
...
\begin{itemize}
  \item transition: issues delete request via M12; user data removed from backend.
...
\subsubsection{Uses}
Repository API Module (M15) [GitHub API, PyPI API] \\
System API Gateway Module (M12) [Django REST Framework, JSON] \\
Database Module (M9) [Django ORM] \\
...
\begin{itemize}
  ...
  \item Persistent storage handled through Database Module (M9) using Django ORM.
\end{itemize}
...
\begin{itemize}
  \item transition: inserts or updates metric entries in M9.
...
\subsubsection{Uses}
Database Configuration Module (M19) [Django ORM, PostgreSQL] \\
System API Gateway Module (M12) [Django REST Framework] \\
Graphing Module (M14) [Chart.js / Plotly via React] \\
Ranking Algorithm Module (M13) [for displaying domain scores] \\
Automated Metrics Module (M8)
...
\begin{itemize}
  \item transition: retrieves the domain’s metric data via M8 and renders a chart through M14.
...
\subsubsection{Uses}
Ranking Algorithm Module (M13) [AHP / BWM weighting methods] \\
Automated Metrics Module (M8) [metrics retrieval] \\
Database Module (M9) [Django ORM – metrics and package data] \\
Graphing Module (M14) [Chart.js / Plotly for visual comparison] \\
System API Gateway Module (M12) [Django REST Framework]  
...
\begin{itemize}
  \item transition: executes the selected comparison algorithm using data fetched via M8 and M9.  
...
\begin{itemize}
  \item transition: renders visual representation of results via M14.  
...
It interacts with M5 (User Authentication) to identify the active user and with M9 (Database Module) for persistent storage of configuration data.

\subsubsection{Uses}
User Authentication Module (M5) [Django authentication system] \\
Database Module (M9) [Django ORM / PostgreSQL] \\
System API Gateway Module (M12) [REST API for data retrieval] \\
...
\begin{itemize}
  ...
  \item REST API endpoints exposed by M12 for frontend communication.  
\end{itemize}
...
\begin{itemize}
  ...
  \item Default values are always retrievable from M20 (Configuration Constants Module).  
\end{itemize}
...
\begin{itemize}
  \item transition: resets all stored configuration to default values from M20.  
...
\begin{itemize}
  ...
  \item \texttt{mergeDefaults()} — merges user-provided configuration with global defaults.  
...
\subsubsection{Uses}
Database Module (M9) [Django ORM / PostgreSQL] \\
User Authentication Module (M5) [Django Auth Middleware] \\
Automated Metrics Module (M8) [API data collection integration] \\
Repository API Module (M17) [external API requests] \\
Configuration Module (M19)
...
\subsubsection{Uses}
Database Module (M9) [metrics and domain data via Django ORM] \\
Automated Metrics Module (M8) [input metric data] \\
Comparison Module (M16) [for result visualization] \\
Configuration Module (M19) [for user-defined weights and preferences]
...
\begin{itemize}
  ...
  \item Django backend provides metric input via M9 and M8.  
  \item React frontend retrieves results from M12 through REST endpoints.  
\end{itemize}
...
It operates as a service module, receiving numerical or categorical data from other modules and outputting visual plots (bar, line, pie, or radar charts) for display or export.

\subsubsection{Uses}
Automated Metrics Module (M8) [provides metric values] \\
Database Module (M9) [retrieves stored dataset entries] \\
System API Gateway Module (M10) [routes graphing requests] \\
Comparison Module (M16) [requests comparative plots] \\
Configuration Module (M19) [retrieves user visualization preferences]
...
\subsubsection{Uses}
Database Module (M9) [for data persistence] \\
System API Gateway Module (M10) [for upload routing and validation] \\
Configuration Module (M19) [for file-format settings] \\
Automated Metrics Module (M8) [for synchronization of imported metrics]  
...
\subsubsection{Uses}
Database Module (M9) [retrieves structured data for export] \\
System API Gateway Module (M10) [handles export requests and delivery] \\
Configuration Module (M19) [provides user-selected export settings] \\
Ranking Algorithm Module (M11) [supplies ranking data for export]
...
\begin{itemize}
  ...
  \item Django backend provides query data via ORM (M9).  
...
\subsubsection{Uses}
Automated Metrics Module (M8) [for metric retrieval requests] \\
System API Gateway Module (M10) [for backend communication] \\
Configuration Module (M19) [for API tokens and environment variables] \\
Database Module (M9) [for storing fetched data]  
...
\subsubsection{Uses}
System API Gateway Module (M10) [for CRUD operations] \\
Configuration Module (M19) [for connection credentials] \\
Automated Metrics Module (M8) [for inserting new metric data] \\
Ranking Algorithm Module (M11) [for reading metric tables during ranking]
...
\subsubsection{Uses}
System API Gateway Module (M10) [for capturing backend requests and responses] \\
Database Module (M17) [for storing error logs and event history] \\
Configuration Module (M19) [for environment-specific logging paths and levels]  
