...
\subsubsection*{Overlap with Existing Tools}\vspace{-0.5em}
There is also the risk that we develop a tool or process that is very similar to something already existing but not well-documented, which could make our contribution seem less original and reduce the project's unique value.
\subsubsection*{Subjectivity of Evaluation Criteria}\vspace{-0.5em}
Some evaluation criteria may rely too much on human judgment. Qualities such as usability, understandability, or maintainability are difficult to measure objectively, and different evaluators may assign different scores to them. This could reduce the reproducibility and reliability of our results, especially if the tool is later applied in other contexts or by different teams.
