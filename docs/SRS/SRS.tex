...
The “methodology to assess the state of best practice for Domain X” paper, outlines a series of systematic steps to access the state of best practice for a particular domain.
...
To assist in the creation of this paper, we will create a reusable tool, whose main purpose is to simplify the process of gathering, visualizing and storing the data. This tool will than be reusable for future domains. 
\subsection{Goals of the Project}
The tool will make the data gathering process faster and more efficient for the current paper used to pilot the project. Future papers will than be able to use the tool to assist in their own data gathering for their domain.\\
...
The tool must be able to extract the repository based metrics automatically, to eliminate the need for user to manually enter this information. At least 80\% of the accessed packages should have these type of metrics automatically filled without error.
\subsubsection{Interactive Data Table}
The tool's data table should be intuitive to use, and be similar to pre-existing industry standard spreadsheet tools such as Excel. A user who have prior experience with Excel should automatically be able to use the tool for inputting data.
...
As well as accessing if the tool makes the current data gathering process easier, as compared to the previous ad-hoc method of using excel. The research subteam will be alternating between the current students working on this project: Awurama Nyarko, Fei Xie, Ghena Hatoum, and Haniye Hamidizadeh.
\subsection{Customer}
