...
\item The tool must include clear setup instructions in a README file so that developers can easily install dependencies, configure the environment, and run both the backend and frontend.
...
\item The system should be deployable on a standard cloud or web-hosting platform, allowing users to access it directly through a web interface without local installation.
...
\item Automated linting tools must remain part of the workflow so that new contributors can easily read, maintain, and update the code.
